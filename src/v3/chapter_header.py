...
from v3.console import Progress, get_console, get_progress_columns
from v3.mongo import Mongo
from v3.unparsed import Unparsed

skip_words: List[str] = ["chapter", "translator", "nyoi-bo", "studio", "editor"]
...
def update_chapter(
    ...
    if doc:
        progress.update(
            task_id, advance=0.1, description=f"Updating Chapter {doc.chapter}..."
        )
        assert chapter == doc.chapter, f"Chapter {chapter} not found in the database."
        doc.update(Set({"title": title})).update(Set({"text": text})).update(
            Set({"trimmed_text": trimmed_text})
        )
        progress.update(
            task_id, advance=0.1, description=f"Chapter {doc.chapter} Updated."
        )
...
def main() -> None:
    ...
    with Progress(
        ...
        mongo = Mongo()
        mongo._connect()
        trim = progress.add_task(
            description="Trimming Chapters Text", total=Unparsed.count()
        )
        for doc in Unparsed.all(sort="chapter"):
            progress.update(
                trim, advance=0.1, description=f"Retrieving Chapter {doc.chapter}..."
            )
            if not doc:
            ...
            progress.update(
                trim,
                advance=0.1,
                description=f"Trimming Chapter {doc.chapter}'s Text...",
            )
            trimmed_text, updated_text = trim_text(doc.text, doc.chapter, doc.title)

            progress.update(
                trim, advance=0.4, description=f"Trimmed Chapter {doc.chapter}..."
            )

            progress.update(
                trim, advance=0.2, description=f"Processing Chapter {doc.chapter}..."
            )
            chapter_doc = Chapter.find_one(Chapter.chapter == doc.chapter).run()
            if not chapter_doc:
                raise ValueError(f"Chapter {doc.chapter} not found in the database.")
            doc.update(Set({"title": doc.title})).update(
                Set({"text": updated_text})
            ).update(Set({"trimmed_text": trimmed_text}))
    ...
    with Progress(
        ...
        with ThreadPoolExecutor(max_workers=cpu_count()) as executor:
            ...
            for doc in Unparsed.all(sort="chapter"):
                if doc.chapter <= 784:
                    progress.update(
                        trim, advance=1, description=f"Trimmed Chapter {doc.chapter}..."
                    )
                    continue
                future = executor.submit(trim_text, doc.text, doc.chapter, doc.title)
                future.add_done_callback(
                    lambda future: progress.update(
                        trim,
                        advance=0.1,
                        description=f"Trimmed Chapter {doc.chapter}...",
                    )
                )
            ...
            for future in as_completed(futures):
                ...
                progress.update(
                    trim,
                    advance=0.2,
                    description=f"Processing Chapter {doc.chapter}...",
                )
                chapter_doc = Chapter.find_one(Chapter.chapter == doc.chapter).run()
                if not chapter_doc:
                    raise ValueError(
                        f"Chapter {doc.chapter} not found in the database."
                    )
                doc.update(Set({"title": doc.title})).update(
                    Set({"text": updated_text})
                ).update(Set({"trimmed_text": trimmed_text}))
                progress.update(
                    trim, advance=0.2, description=f"Chapter {doc.chapter} Updated."
                )
                progress.update(
                    trim, advance=0.2, description=f"Trimmed Chapter {doc.chapter}..."
                )
                progress.update(
                    trim,
                    advance=0.4,
