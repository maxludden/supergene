...
from v3.console import Console, Progress, get_console
...
def main() -> None:
    ...
    with Progress(
        ...
        for k, v in read_toc().items():
            progress.update(
                update_titles, advance=0.6, description=f"Updating Chapter {k}..."
            )
            chapter: int = int(k)
            title: str = v
            result = update_chapter(chapter, title, progress)  # type: ignore # noqa: F841
            progress.update(
                update_titles, advance=0.4, description=f"Chapter {k} Updated."
            )
        ...
        with ThreadPoolExecutor(max_workers=cpu_count()) as executor:
            ...
            for k, v in read_toc().items():
                ...
                future: Future = executor.submit(
                    update_chapter, chapter, title, progress
                )
                futures.append(future)
            for index, future in enumerate(as_completed(futures)):
                progress.update(
                    update_titles, advance=1, description=f"Chapter {k} Updated."
                )
